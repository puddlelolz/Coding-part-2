...
* `todo_app` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `todo_app` - executable or bundle name on disk.
* `todo_app` - display name shown in window titles and about app dialogs.
* `com.puddle` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
